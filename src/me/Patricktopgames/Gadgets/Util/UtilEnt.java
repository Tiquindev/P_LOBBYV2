...
implements Listener{
	...
	private static HashMap<org.bukkit.entity.Entity, String> _nameMap = new HashMap();
	  ...
	  public static void removeGoalSelectors(org.bukkit.entity.Entity paramEntity)
	  {
	    try
	    {
	      ...
	      if ((((CraftEntity)paramEntity).getHandle() instanceof EntityInsentient))
	      {
	        ...
	        PathfinderGoalSelector localPathfinderGoalSelector = new PathfinderGoalSelector(((CraftWorld)paramEntity.getWorld()).getHandle().methodProfiler);
	  ...
	  {
	    if (UtilMath.offset(paramLocation, paramLivingEntity.getLocation().add(0.0D, 0.4D, 0.0D)) < 0.6D * paramDouble) {
	      return true;
	    }
	    if ((paramLivingEntity instanceof Player))
	    {
	      ...
	      if ((UtilMath.offset2d(paramLocation, localPlayer.getLocation()) < 0.6D * paramDouble) && 
	        (paramLocation.getY() > localPlayer.getLocation().getY()) && (paramLocation.getY() < localPlayer.getEyeLocation().getY())) {
	        return true;
	      }
	    }
	    else if ((paramLivingEntity instanceof Giant))
	    {
	      if ((paramLocation.getY() > paramLivingEntity.getLocation().getY()) && (paramLocation.getY() < paramLivingEntity.getLocation().getY() + 12.0D) && 
	        (UtilMath.offset2d(paramLocation, paramLivingEntity.getLocation()) < 4.0D)) {
	        return true;
	      }
	    }
	    else if ((paramLocation.getY() > paramLivingEntity.getLocation().getY()) && (paramLocation.getY() < paramLivingEntity.getLocation().getY() + 2.0D) && 
	  ...
	  public static boolean hitBox(Location paramLocation, org.bukkit.entity.Entity paramEntity, double paramDouble)
	  {
	    ...
	    {
	      if ((paramLocation.getY() > paramEntity.getLocation().getY()) && (paramLocation.getY() < paramEntity.getLocation().getY() + 12.0D) && 
	        (UtilMath.offset2d(paramLocation, paramEntity.getLocation()) < 4.0D)) {
	        return true;
	      }
	    }
	    else if ((paramLocation.getY() > paramEntity.getLocation().getY()) && (paramLocation.getY() < paramEntity.getLocation().getY() + 2.0D) && 
	  ...
	  public static boolean isGrounded(org.bukkit.entity.Entity paramEntity)
	  {
	    ...
	    return Main.getUtilBlock().solid(paramEntity.getLocation().getBlock().getRelative(BlockFace.DOWN));
	  }
	  ...
	  public static void PlayDamageSound(LivingEntity paramLivingEntity)
	  {
	    ...
	    paramLivingEntity.getWorld().playSound(paramLivingEntity.getLocation(), localSound, 1.5F + (float)(0.5D * Math.random()), 0.8F + (float)(0.4000000059604645D * Math.random()));
	  }
	  ...
	  public static boolean CreatureMoveFast(org.bukkit.entity.Entity paramEntity, Location paramLocation, float paramFloat)
	  {
	    if (!(paramEntity instanceof Creature)) {
	      return false;
	    }
	    ...
	    EntityCreature localEntityCreature = ((CraftCreature)paramEntity).getHandle();
	    localEntityCreature.getControllerMove().a(paramLocation.getX(), paramLocation.getY(), paramLocation.getZ(), paramFloat);
	    
	    return true;
	  }
	  
	  public static void setHelmet(LivingEntity paramLivingEntity, ItemStack paramItemStack)
	  {
	    EntityEquipment localEntityEquipment = paramLivingEntity.getEquipment();
	    localEntityEquipment.setHelmet(paramItemStack);
	  }
	  
	  public static void setItemInHand(LivingEntity paramLivingEntity, ItemStack paramItemStack)
	  {
	    EntityEquipment localEntityEquipment = paramLivingEntity.getEquipment();
	  ...
	  {
	    ItemStack item = new ItemStack(tipo, 1, id);
	    
	    Item drop = l.getWorld().dropItem(l, item);
